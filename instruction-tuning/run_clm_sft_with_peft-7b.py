...
import datasets
import torch
from build_dataset import build_instruction_dataset, build_instruction_dataset_line, build_instruction_dataset_space, DataCollatorForSupervisedDataset
import transformers
...
def main():
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            if 'CoT' in data_args.dataset_dir:
                train_dataset = build_instruction_dataset_line(
                data_path=files,
                tokenizer=tokenizer,
                max_seq_length=data_args.max_seq_length,
                data_cache_dir = None,
                preprocessing_num_workers = data_args.preprocessing_num_workers)
            if 'kg-to-text' in data_args.dataset_dir or 'summary' in data_args.dataset_dir:
                ...
                if 'Llama-3' in model_args.model_name_or_path:
                    train_dataset = build_instruction_dataset_space(
                    data_path=files,
                    tokenizer=tokenizer,
                    max_seq_length=data_args.max_seq_length,
                    data_cache_dir = None,
    ...
    if training_args.do_eval:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            if 'CoT' in data_args.dataset_dir:
                eval_dataset = build_instruction_dataset_line(
                data_path=files,
                tokenizer=tokenizer,
                max_seq_length=data_args.max_seq_length,
                data_cache_dir = None,
            ...
            if 'kg-to-text' in data_args.dataset_dir or 'summary' in data_args.dataset_dir:
                if 'llama-2' in model_args.model_name_or_path:
                    ...
                    preprocessing_num_workers = data_args.preprocessing_num_workers)
                if 'Llama-3' in model_args.model_name_or_path:
                    eval_dataset = build_instruction_dataset_space(
                    data_path=files,
                    tokenizer=tokenizer,
                    max_seq_length=data_args.max_seq_length,
                    data_cache_dir = None,
