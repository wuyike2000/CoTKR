...
# get 1 hop relation
def query_1hop_relation(head,kb_endpoint = kb_endpoint):
    skip=['type.object.name','type.object.type','common.topic.description','common.topic.notable_for','common.topic.article','common.topic.topic_equivalent_webpage','common.topic.alias',"common.topic.webpage",'common.topic.topical_webpage',"common.topic.notable_types","type.object.key","kg.object_profile.prominent_type"]
    results = set()
    query = "PREFIX ns: <http://rdf.freebase.com/ns/> SELECT ?rel WHERE { ns:"+ head + " ?rel ?y . FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/common.topic')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/type.object')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/key/')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/freebase')) FILTER (!STRENDS(STR(?rel), 'type')) FILTER (!STRENDS(STR(?rel), 'label'))}"
    query_results = KB_query(query, kb_endpoint)
    for i in query_results:
        if "http://rdf.freebase.com/ns/" not in i['rel']:
            continue
...
# get 2 hop relation
def query_2hop_relation(head,kb_endpoint = kb_endpoint):
    skip=['type.object.name','type.object.type','common.topic.description','common.topic.notable_for','common.topic.article','common.topic.topic_equivalent_webpage','common.topic.alias',"common.topic.webpage",'common.topic.topical_webpage',"common.topic.notable_types","type.object.key","kg.object_profile.prominent_type"]
    results = set()
    query = "PREFIX ns: <http://rdf.freebase.com/ns/> SELECT ?rel ?rel1 WHERE { ns:"+ head + " ?rel ?y . ?y ?rel1 ?z . FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/common.topic')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/type.object')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/key/')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/freebase')) FILTER (!STRENDS(STR(?rel), 'type')) FILTER (!STRENDS(STR(?rel), 'label')) FILTER (!STRSTARTS(STR(?rel1), 'http://rdf.freebase.com/ns/common.topic')) FILTER (!STRSTARTS(STR(?rel1), 'http://rdf.freebase.com/ns/type.object')) FILTER (!STRSTARTS(STR(?rel1), 'http://rdf.freebase.com/key/')) FILTER (!STRSTARTS(STR(?rel1), 'http://rdf.freebase.com/ns/freebase')) FILTER (!STRENDS(STR(?rel1), 'type')) FILTER (!STRENDS(STR(?rel1), 'label'))}"
    query_results = KB_query(query, kb_endpoint)
    for i in query_results:
        if "http://rdf.freebase.com/ns/" not in i['rel'] or "http://rdf.freebase.com/ns/" not in i['rel1']:
            continue
        rel=i['rel'].replace("http://rdf.freebase.com/ns/","")
        rel1=i['rel1'].replace("http://rdf.freebase.com/ns/","")
        if rel in skip or rel1 in skip:
            continue
        results.add((rel,rel1))
    return results
    
def get_1hop_chain(head,relation,kb_endpoint = kb_endpoint):
    query = "PREFIX ns: <http://rdf.freebase.com/ns/> SELECT ?y WHERE { ns:"+ head + " ns:" + relation + " ?y .}"
    results = KB_query(query, kb_endpoint)
    triples=[]
    for i in results:
        y=i['y'].replace("http://rdf.freebase.com/ns/","").replace('-08:00','')
        triples.append([head,relation,y])
    return triples
    
def get_2hop_chain(head,rechain,kb_endpoint = kb_endpoint):
    query = "PREFIX ns: <http://rdf.freebase.com/ns/> SELECT ?y ?z WHERE { ns:"+ head + " ns:" + rechain[0] + " ?y . ?y ns:"+ rechain[1]+" ?z .}"
    results = KB_query(query, kb_endpoint)
    triples=[]
    for i in results:
        y=i['y'].replace("http://rdf.freebase.com/ns/","").replace('-08:00','')
        z=i['z'].replace("http://rdf.freebase.com/ns/","").replace('-08:00','')
        triples.append([head,rechain[0],y])
        triples.append([y,rechain[1],z])
    return triples
    
def get_2hop_triples(head, kb_endpoint = kb_endpoint):
    results = []
    query = "PREFIX ns: <http://rdf.freebase.com/ns/> SELECT ?rel ?y WHERE { ns:"+ head + " ?rel ?y . FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/common.topic')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/type.object')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/key/')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/freebase')) FILTER (!STRENDS(STR(?rel), 'type')) FILTER (!STRENDS(STR(?rel), 'label'))} LIMIT 1000"
    query_results = KB_query(query, kb_endpoint)
    query=set()
    for i in query_results:
        y=i['y'].replace("http://rdf.freebase.com/ns/","").replace('-08:00','')
        rel=i['rel'].replace("http://rdf.freebase.com/ns/","").replace('-08:00','')
        temp=[head,rel,y]
        results.append(temp)
        # add y for further query if y is an entity
        if y[0:2] in ['m.','n.','g.']:
            query.add(y)
    for que in query:
        query = "PREFIX ns: <http://rdf.freebase.com/ns/> SELECT ?rel ?y WHERE { ns:"+ que + " ?rel ?y . FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/common.topic')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/type.object')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/key/')) FILTER (!STRSTARTS(STR(?rel), 'http://rdf.freebase.com/ns/freebase')) FILTER (!STRENDS(STR(?rel), 'type')) FILTER (!STRENDS(STR(?rel), 'label'))} LIMIT 1000"
        query_results = KB_query(query, kb_endpoint)
        for i in query_results:
            y=i['y'].replace("http://rdf.freebase.com/ns/","").replace('-08:00','')
            rel=i['rel'].replace("http://rdf.freebase.com/ns/","").replace('-08:00','')
...
if __name__ == '__main__':
    ...
    #print(query_2hop_relation('m.02hvt2s'))
    #print(get_1hop_chain('m.0_qxwd5','award.award_nomination.award')) 
    #print(get_2hop_chain('m.0_qxwd5',['award.award_nomination.award','award.award_category.presenting_organization']))  
    print(get_2hop_triples('m.0_qxwd5'))