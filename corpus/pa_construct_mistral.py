...
# path for ANS
ANS_PATH='../../pretrain/'+ANS
# path for tokenizer
ANS_TOKENIZER='../../pretrain/'+ANS
# path for lora
PEFT_PATH='../instruction-tuning/output-'+DATA+'/'+MODE+'/'+LLM+'/best_model'
# result for checking
result=DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/sample.json'
# DPO data construction
DPO_data=DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/DPO.json'
DPO_train=DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/train.json'
DPO_dev=DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/dev.json'
os.makedirs(DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/middle',exist_ok = True)
...
ans_llm=AutoModelForCausalLM.from_pretrained(ANS_PATH,torch_dtype=torch.float16,low_cpu_mem_usage=True,device_map='cuda:1')
...
index=0
if resume>0:
    record=json.load(open(DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/middle/sample-'+str(resume)+'.json','r',encoding='utf-8'))
    DPO=json.load(open(DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/middle/DPO-'+str(resume)+'.json','r',encoding='utf-8'))
    index=resume
...
with torch.no_grad():
    for sample in tqdm(data[resume:]):
        ...
        if index%INTERVAL==0:
            ...
            result_interval=DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/middle/sample-'+str(index)+'.json'
            # path for interval dpo data
            DPO_data_interval=DATA+'/PA-Mistral/'+MODE+'/'+LLM+'/middle/DPO-'+str(index)+'.json'
            # save
        ...
        ans_list=[]
        for i in know_list:
            ans_list.append(LLMSingleResponse(ans_prompt.format(knowledge=i.strip(),ques=sample["question"]).strip(),ans_llm,ans_tokenizer,'cuda:1'))
